...
class AutoThetaBackend(ForecastingBackend):
    def __init__(self):
        short_description = "Automatic Theta method for forecasting."
        long_description = ""
        super().__init__(
            'auto-theta',
            short_description=short_description,
            long_description=long_description,
            callback_url=create_callback_url('forecasting', 'auto-theta'),
            parameters=[
                NumberParameter(
                    name='horizon',
    ...
    def do_analysis(self, data: pd.DataFrame, feature: str, **kwargs):
        ...
        result = pd.DataFrame({
            ...
            'forecast': forecast['AutoTheta'].values,
            'lo': forecast['AutoTheta-lo-95'].values,
            'hi': forecast['AutoTheta-hi-95'].values
        })
        return result
